...
Root_dir = '/home/nan/Dropbox/PR2019/author_image'
Target_dir = '/home/nan/Dropbox/PR2019/autho_image_resize'
...
#             img = cv2.resize(img, (285,336))
#             targte_path = os.path.join(Target_dir, sub1, sub2, img_name)
#             cv2.imwrite(targte_path, img)

sub2_list = os.listdir(os.path.join(Root_dir))
for img_name in sub2_list:
        img_path = os.path.join(Root_dir, img_name)
        img = cv2.imread(img_path)
        img = cv2.resize(img, (285,336))
        targte_path = os.path.join(Target_dir,  img_name)
        cv2.imwrite(targte_path, img)
