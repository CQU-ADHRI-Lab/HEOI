...
class TBVisualizer:
    def __init__(self, opt):
        ...
        mkdir(os.path.join(opt.checkpoints_dir, opt.name, 'tensorboard'))
        self._save_path = os.path.join(opt.checkpoints_dir, opt.name,'tensorboard')

        self._log_path = os.path.join(self._save_path, 'log.txt')
        ...
        for label, image_numpy in visuals.items():
            ...
            if save_visuals:
                util.save_image(image_numpy,
                                os.path.join(self._opt.checkpoints_dir,
                                             self._opt.name, 'event_imgs',
                                             sum_name, '%08d.png' % it))
    ...
    def print_current_train_errors(self, epoch, i, iters_per_epoch, errors, t,
        ...
        message = '%s (Training epoch: %d, iter/batch_num: %d/%d, ' \
                  'time/sample: %.3fs)\n ' % (log_time, epoch, i,
                                              iters_per_epoch, t)
    ...
    def print_current_validate_errors(self, epoch, errors, t):
        log_time = time.strftime("[%d/%m/%Y %H:%M:%S]")
        message = '%s (Validation, epoch: %d, time_to_val: %ds) ' % (log_time,
                                                                     epoch, t)
        for k, v in errors.items():
            message += '%s:%.3f ' % (k, v)
